...
public:
    ...
    LRESULT (CALLBACK*WndProc)(HWND, UINT, WPARAM, LPARAM) = 0;
    LPTSTR hIcon;
    ...
    int CreateExtensionMain()
    {
        ...
        else { MessageBox(0, 0, 0, 0); return -1; }; if (this->CheckExtension != false) { if (FindWindow(L"LIGHTFRAME", L"LightFrame")) { HWND h = FindWindow(L"LIGHTFRAME", L"LightFrame"); PostMessage(h, LFM_EXTENSION, 0, 0); } else { if ((this->DependOnLightFrameMain == true))return -1; } }HWND hWnd; if (this->OnCreateWindow != __noop) { hWnd = this->OnCreateWindow(); } else{hWnd= CreateWindow(this->szWindowClass, this->szTitle,this->wStyle, this->x, this->y, this->Width, this->Height, nullptr, nullptr, this->hInst, nullptr); this->hWnd = hWnd;}
        if (!hWnd){return FALSE;}
        if (this->NoOuterFrame == 1) { LONG_PTR Style = ::GetWindowLongPtr(hWnd, GWL_STYLE); Style = Style & ~WS_CAPTION & ~WS_SYSMENU & ~WS_SIZEBOX; ::SetWindowLongPtr(hWnd, GWL_STYLE, Style); ::SetWindowLong(hWnd, GWL_EXSTYLE, GetWindowLong(hWnd, GWL_EXSTYLE) & ~WS_EX_APPWINDOW | WS_EX_TOOLWINDOW); ::SetWindowPos(hWnd, 0, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE); }ShowWindow(hWnd, this->ShowFlag); if (this->OnCreateCmd != __noop) { this->OnCreateCmd(); SendMessage(hWnd, LFM_ONCREATEEXTENSION, 0, 0); }
