	...
	namespace Window
	{
		namespace OuterShadow
		{
			...
			void DropShadow::MakeShadow(UINT* pShadBits, HWND hWnd, RECT* rcParent)
			{
				...
				for (int i = min(nKernelSize, max(gnSize - gnyOffset, 0));
					i < max(szShadow.cy - nKernelSize, min(szParent.cy + gnSize - gnyOffset, szParent.cy + 2 * gnSize));
					i++)
				{
					...
					else
					{
						for (int j = ptAnchors[i][0];
							j < min(ptAnchorsOri[i - gnSize + gnyOffset][0] + gnSize - gnxOffset, ptAnchors[i][1]);
							j++)
							*(pLine + j) = clCenter;
						for (int j = max(ptAnchorsOri[i - gnSize + gnyOffset][0] + gnSize - gnxOffset, 0);
							j < min(ptAnchorsOri[i - gnSize + gnyOffset][1] + gnSize - gnxOffset, szShadow.cx);
							j++)
							*(pLine + j) = 0;
						for (int j = max(ptAnchorsOri[i - gnSize + gnyOffset][1] + gnSize - gnxOffset, ptAnchors[i][0]);
							j < ptAnchors[i][1];
							j++)
							*(pLine + j) = clCenter;
					}
		...
		namespace Slider
		{
			...
			VERTEXUICTL VUIW_Slider_OKButton;
			RUNFUN VUIW_Slider_OKButton_Event()
			{
				SliderValue = ReturnValue;
				DestroyWindow(hSliderWnd);
				PostQuitMessage(0);
				return 0;
			}
			...
			int hSliderWindowAreaEvent(HWND hWnd, LPARAM lParam)
			{
				...
				RECT rc = {};
				if (GetAreaPtInfo(hWnd, 40, 60, 302, 70, RECT{ 0 }, lParam))
				{
					if (KeepDownMsg == 1)
					{
						barVal = LOWORD(lParam) - 40;
				...
				AddCtlEvent(hWnd, lParam, VUIW_Slider_OKButton)
			...
			INT_PTR CALLBACK CreateManifestWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
			{
				...
				case WM_PAINT:
				{
					...
					_itow(ReturnValue, ValueChar, 10);
					TextPreDrawEx(hMemDC, 45, 95, 100, 30, ValueChar, 22, 0, VERTEXUICOLOR_WHITE);
					CreateRect(hWnd, hMemDC, 40, 80, RCWID - 80, 5, VERTEXUICOLOR_LDARK);
					if (ReturnValue > 99) { CreateRect(hWnd, hMemDC, barVal + 35, 72, 10, 22, VERTEXUICOLOR_GREENSEA); }
					else { CreateRect(hWnd, hMemDC, barVal + 40, 72, 10, 22, VERTEXUICOLOR_GREENSEA); }

					VUIW_Slider_OKButton = InitCtl(L"Button", (RCWID - 160) / 2, rc.bottom - 50, 160, 30, VUIW_Slider_OKButton_Event, L"确定");
					CreateCtl(hWnd, hMemDC, VUIW_Slider_OKButton);
		...
		namespace TimeSelector
		{
			...
			INT_PTR CALLBACK CreateManifestWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
			{
				...
				case WM_PAINT:
				{
					...
					min->Set(31+(100+20)*1, 30, 100, 140,1);
					...
					ok.CreateCtl(L"Button2", 271, rc.bottom - 50, 100, 40, [hWnd] {
						int val;
						val = hrs->value * 60 * 60 + min->value * 60 + sec->value;
						gtime = val;
